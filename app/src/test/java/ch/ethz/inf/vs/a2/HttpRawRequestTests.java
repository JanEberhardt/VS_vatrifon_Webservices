...
public class HttpRawRequestTests{
	...
	public void acceptHeader() {
		...
		for (int i = 1; i < split.length; i++) {
			...
			if (matcher.group(1).equals("Accept")) {
				...
				Assert.assertTrue("\"" + accept + "\" does not match expected value \"close\".", accept.equals("text/html"));
				break;
			}
		}
	...
	private String createRequest(){
		HttpRawRequest request = HttpRawRequestFactory.getInstance(TEST_HOST, TEST_PORT, TEST_PATH);
